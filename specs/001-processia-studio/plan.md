...
T010 ajoute un diagnostic privé rattaché à la révision courante du modèle et à une divergence T009. Son périmètre nomme les tâches étudiées et sa limite de couverture. Une opportunité conserve séparément valeur attendue, faisabilité, prérequis, priorité proposée, responsable humain et protocole d'essai. Une dimension absente reste nulle et Inconnue ; aucun score n'est calculé tant que DEC-07 reste ouvert. La feuille de route place les prérequis avant l'essai et n'exécute aucun agent. Un scénario cible séparé référence la révision et le libellé de départ d'une tâche. Sa validation humaine ne modifie pas le modèle actuel ; toute révision ultérieure déclenche un état de réconciliation. Le diagnostic possède sa propre version et une révision manuelle de priorité conserve l'avant, l'après, l'auteur et la justification. Une architecture conceptuelle relie une capacité mutualisable à plusieurs usages. Chaque liaison garde son contexte, ses tâches et son périmètre d'accès ; aucun accès aux données n'est mis en commun. Le fournisseur reste nul et l'exécution désactivée. Le suivi d'essai conserve séparément l'hypothèse de gain et chaque mesure observée, avec une unité commune, une méthode, une date et l'auteur issu de la session. Une action d'autonomie dépend de l'essai, cible un rôle connu, décrit un guide ou un exercice et porte son critère de sortie observable. Le studio expose le diagnostic, la capacité, la comparaison, l'historique, ces preuves de gain et la trajectoire vers l'autonomie. Les autres types de changements cibles restent ouverts.
