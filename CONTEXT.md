...
Mathieu demande de lancer puis de poursuivre le développement avec le dépôt GitHub ProcessIA. T001 à T005 utilisent des données synthétiques et un adaptateur de langage simulé. TypeScript, React, Vite, React Flow et SQLite constituent un choix local réversible décrit dans ADR-0001. Les cinq tranches sont vérifiées localement au 10 septembre 2026 : canevas persisté, commandes versionnées, dossiers distincts, accès bornés, fiches de tâches, ingestion textuelle privée via MCP stdio et partage explicite d'une reformulation filtrée. Cette demande ne vaut pas ratification globale des propositions ni choix des fournisseurs ou de l'identité du pilote externe.

Le paragraphe de statut v0.2 ci-dessus décrit l'état au moment de la rédaction. L'état courant, les preuves et les limites sont désormais dans `IMPLEMENTATION_PLAN.md` et les fiches `docs/validation/T001.md` à `T005.md`.
