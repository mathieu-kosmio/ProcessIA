...
- Stade : développement local démarré le 9 septembre 2026, tranches T001 à T006 vérifiées, socle T007, noyaux T008 et T009, puis diagnostic, cible, capacité mutualisable, mesure d'essai et action d'autonomie T010 vérifiés avec leurs limites documentées.
...
Mathieu demande de lancer puis de poursuivre le développement avec le dépôt GitHub ProcessIA. T001 à T010 utilisent des données synthétiques et un adaptateur de langage simulé. TypeScript, React, Vite, React Flow et SQLite constituent un choix local réversible décrit dans ADR-0001. T001 à T006 sont vérifiées localement. T007 possède un socle vérifié pour la continuité voix-texte, sans reconnaissance ni synthèse vocale réelle. T008 possède un noyau BPMN vérifié. T009 conserve des assertions contradictoires avec leurs passages privés et les laisse ouvertes. T010 relie cette divergence à un diagnostic circonscrit, une opportunité proposée et un essai précédé d'un prérequis. Il compare aussi le fonctionnement actuel à une cible validée par une personne, sans modifier le modèle réel, et demande une réconciliation après une nouvelle révision. Une priorité proposée peut être remplacée par une décision manuelle motivée dont l'auteur et l'historique sont conservés. Une capacité conceptuelle mutualisable peut relier plusieurs usages, mais chaque liaison conserve son contexte, ses tâches et son périmètre privé ou partagé. Aucun fournisseur n'est choisi et l'exécution reste désactivée. Le suivi d'essai distingue l'hypothèse de gain et le résultat observé avec valeur, unité, méthode, date et auteur, sans écraser la première valeur. La feuille de route prolonge l'essai par une action d'autonomie ciblant un rôle connu, un guide ou un exercice et un critère observable. Une faisabilité absente reste inconnue et aucun score pondéré n'est calculé. Cette demande ne vaut pas ratification globale des propositions ni choix des fournisseurs, de la méthode de priorité ou de l'identité du pilote externe.

Le paragraphe de statut v0.2 ci-dessus décrit l'état au moment de la rédaction. L'état courant, les preuves et les limites sont désormais dans `IMPLEMENTATION_PLAN.md` et les fiches `docs/validation/T001.md` à `T010.md`.
